...
import { createTask, patchTask, deleteTask, addComment, addAttachments, getTasks, createApprovalTask } from 'store/tasks/actions.tasks';
...
function AddTaskSidebar({ isOpen, setNewTaskOpen, selectedTask, setSelectedTask, streamId }) {
  ...
  const addAssigneeDropdown = (
    <div className={styles.addAssigneeDropdown}>
      ...
      <br />
      {users
        .filter((usr) => usr.streamList.some((stream) => stream.id === streamId))
        .map((usr, i) => {
  ...
  const createNewTask = () => {
    const data = {
      title: taskName,
      type: 'TASK',
      priority: taskPriority.toLocaleUpperCase(),
      status: taskStatus.toLocaleUpperCase(),
      due: dueDate || undefined,
      streamID: streamId,
      description: description || undefined,
    };

    if (taskGroup) data.groupID = taskGroup;

    const assigneeList = assignee.length > 0 ? assignee.map((ass) => ass.id) : [user.id];
    const commentsList = comments.map((comment) => comment.content.props.children);

    dispatch(createTask(data, assigneeList, commentsList, attachments));
  ...
  const createApproval = () => {
    const data = {
      ...
      streamID: streamId,
      description: description || undefined,
      comments: comments.map((comment) => comment.content.props.children),
      attachments,
    };

    if (taskGroup) data.groupID = taskGroup;

    const approver = assignee.length > 0 ? assignee : [user];

    dispatch(createApprovalTask(approvalDocument, approver, data));
  ...
  useEffect(() => {
    ...
    if (isOpen === false) {
      ...
      if (approvalDocument) return createApproval();
      if (taskName !== '' && !selectedTask) return createNewTask();
      ...
      {taskMenu === 'details' && (
        ...
        >
          <Form.Item label="Stream" name="stream" className={styles.formItem}>
            <Badge color={currentStream?.color} text={currentStream?.title} style={{ width: 160 }} />
          </Form.Item>
          ...
          >
            {!selectedTask ? (
              <Form.Item name="document" noStyle>
                ...
                >
                  {documents
                    .filter((document) => document.stream.id === currentStream.id)
                    .map((document, index) => {
                      return (
